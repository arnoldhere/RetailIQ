...
router.get('/get-product-categories', authMiddleware, async (req, res) => {
    try {
        if (!req.user) return res.status(403).json({ message: 'Forbidden' });
        return UserController.getProductCategories(req, res);
    } catch (err) {
        console.log("Error in fetching categories for user...  ", err)
...
router.get("/get-wishlist", authMiddleware, async (req, res) => {
    try {
        ...
        console.log("Error in fetching wishlist for user...  ", err)
        return res.status(500).json({ message: "internal server error" })
    }
})
